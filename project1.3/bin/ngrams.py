...
# from begin up to cutoff. If the list is to include the first 500 values, the
# function call should have a begin parameter value of 0 and a cutoff parameter
# value of 500. In order to include all n-grams (i.e., no cut-off), pass in -1 for
...
def top_ngrams_list_maker(ngrams_fdist_dict_list, begin, cutoff):
	...
	if cutoff == -1 and begin >= 0:
		top_ngrams_fdist_dict_list = sorted_ngrams_fdist_dict_list[begin:]
	elif begin >= 0 and cutoff >= 0:
		top_ngrams_fdist_dict_list = sorted_ngrams_fdist_dict_list[begin:cutoff]
	else:
		sys.exit(1)
	top_ngrams_list = []
...
def main():
	if len(sys.argv) != 17:
		print('usage: ./ngrams_2.py ~/devset/suicide/artist/files/dir/ ~/devset/non-suicide/artist/files/dir/ ~/test-or-training-songs/suicide-or-non-suicide/dir/ ~/n-grams/run-number-and-name/n-gram_stats.txt begin1 cutoff1 begin2 cutoff2 begin3 cutoff3 begin4 cutoff4 begin5 cutoff5 begin6 cutoff6')
		print('You can of course optionally pipe the results to a file by tacking on \"> ~/n-grams/run-number-and-name/suicide-or-non-suicide_test_or_training.csv\"')
		sys.exit(1)
	...
	print('file', end="\t")
	print('diff between sums across all n-grams', end="\t")
	...
	print('diff for 1-gram %s', end="\t")
	print('diff for 2-gram %s', end="\t")
	print('diff for 3-gram %s', end="\t")
	print('diff for 4-gram %s', end="\t")
	print('diff for 5-gram %s', end="\t")
	print('diff for 6-gram %s', end="\t")
	print('average for all suicide n-gram %s', end="\t")
	print('average for all non-suicide n-gram %s', end="\t")
	print('diff between n-gram % averages', end="\t")
#	print('suicide 1-grams', end="\t")
#	print('suicide 2-grams', end="\t")
#	print('suicide 3-grams', end="\t")
#	print('suicide 4-grams', end="\t")
...
#	print('non-suicide 6-grams', end="")
	...
	for song_file in songs_files:
		...
		# Lists of unique n-gram types
		file_unigram_types = list_uniq(file_unigram_profile)
		file_bigram_types = list_uniq(file_bigram_profile)
		file_trigram_types = list_uniq(file_trigram_profile)
		file_four_gram_types = list_uniq(file_four_gram_profile)
		file_five_gram_types = list_uniq(file_five_gram_profile)
		file_six_gram_types = list_uniq(file_six_gram_profile)
		
		# Number of n-gram tokens in file
		total_unigram_tokens = len(file_unigram_profile)
		total_bigram_tokens = len(file_bigram_profile)
		total_trigram_tokens = len(file_trigram_profile)
		total_four_gram_tokens = len(file_four_gram_profile)
		total_five_gram_tokens = len(file_five_gram_profile)
		total_six_gram_tokens = len(file_six_gram_profile)
		
		# Number of n-gram types in file
		total_unigram_types = len(file_unigram_types)
		total_bigram_types = len(file_bigram_types)
		total_trigram_types = len(file_trigram_types)
		total_four_gram_types = len(file_four_gram_types)
		total_five_gram_types = len(file_five_gram_types)
		total_six_gram_types = len(file_six_gram_types)
		...
		for unigram in file_unigram_types:
		...
		for bigram in file_bigram_types:
		...
		for trigram in file_trigram_types:
		...
		for four_gram in file_four_gram_types:
		...
		for five_gram in file_five_gram_types:
		...
		for six_gram in file_six_gram_types:
		...
		# Feature values: (total n-grams in exclusive profiles)/(total n-gram types)
		ratio_suicide_exclusive_unigrams = float(total_unigrams_in_suicide_exclusive)/float(total_unigram_types)
		ratio_suicide_exclusive_bigrams = float(total_bigrams_in_suicide_exclusive)/float(total_bigram_types)
		ratio_suicide_exclusive_trigrams = float(total_trigrams_in_suicide_exclusive)/float(total_trigram_types)
		ratio_suicide_exclusive_four_grams = float(total_four_grams_in_suicide_exclusive)/float(total_four_gram_types)
		ratio_suicide_exclusive_five_grams = float(total_five_grams_in_suicide_exclusive)/float(total_five_gram_types)
		ratio_suicide_exclusive_six_grams = float(total_six_grams_in_suicide_exclusive)/float(total_six_gram_types)
		ratio_nonsuicide_exclusive_unigrams = float(total_unigrams_in_nonsuicide_exclusive)/float(total_unigram_types)
		ratio_nonsuicide_exclusive_bigrams = float(total_bigrams_in_nonsuicide_exclusive)/float(total_bigram_types)
		ratio_nonsuicide_exclusive_trigrams = float(total_trigrams_in_nonsuicide_exclusive)/float(total_trigram_types)
		ratio_nonsuicide_exclusive_four_grams = float(total_four_grams_in_nonsuicide_exclusive)/float(total_four_gram_types)
		ratio_nonsuicide_exclusive_five_grams = float(total_five_grams_in_nonsuicide_exclusive)/float(total_five_gram_types)
		ratio_nonsuicide_exclusive_six_grams = float(total_six_grams_in_nonsuicide_exclusive)/float(total_six_gram_types)
		
		# Sums of n-gram percentages
		
		suicide_sum_of_ngram_percentages = ratio_suicide_exclusive_unigrams + ratio_suicide_exclusive_bigrams + ratio_suicide_exclusive_trigrams + ratio_suicide_exclusive_four_grams + ratio_suicide_exclusive_five_grams + ratio_suicide_exclusive_six_grams
		
		nonsuicide_sum_of_ngram_percentages = ratio_nonsuicide_exclusive_unigrams + ratio_nonsuicide_exclusive_bigrams + ratio_nonsuicide_exclusive_trigrams + ratio_nonsuicide_exclusive_four_grams + ratio_nonsuicide_exclusive_five_grams + ratio_nonsuicide_exclusive_six_grams
		
		# Difference between sums of n-gram percentages (suicide sum minus
		# non-suicide sum)
		diff_between_sums_across_all_ngrams = suicide_sum_of_ngram_percentages - nonsuicide_sum_of_ngram_percentages
		
		# Differences between suicide and non-suicide n-gram percentages
		diff_for_unigram_percentages = ratio_suicide_exclusive_unigrams - ratio_nonsuicide_exclusive_unigrams
		diff_for_bigram_percentages = ratio_suicide_exclusive_bigrams - ratio_nonsuicide_exclusive_bigrams
		diff_for_trigram_percentages = ratio_suicide_exclusive_trigrams - ratio_nonsuicide_exclusive_trigrams
		diff_for_four_gram_percentages = ratio_suicide_exclusive_four_grams - ratio_nonsuicide_exclusive_four_grams
		diff_for_five_gram_percentages = ratio_suicide_exclusive_five_grams - ratio_nonsuicide_exclusive_five_grams
		diff_for_six_gram_percentages = ratio_suicide_exclusive_six_grams - ratio_nonsuicide_exclusive_six_grams
		
		# Averages for all n-gram percentages
		average_suicide_ngram_percentage = float(suicide_sum_of_ngram_percentages)/float(6)
		average_nonsuicide_ngram_percentage = float(nonsuicide_sum_of_ngram_percentages)/float(6)
		
		# Difference between suicide and non-suicide n-gram percentage averages
		# (suicide minus non-suicide)
		diff_between_ngram_percentage_averages = average_suicide_ngram_percentage - average_nonsuicide_ngram_percentage
		
		# Print the feature values
		print(song_file, end="\t")
		print(diff_between_sums_across_all_ngrams, end="\t")
		...
		print(diff_for_unigram_percentages, end="\t")
		print(diff_for_bigram_percentages, end="\t")
		print(diff_for_trigram_percentages, end="\t")
		print(diff_for_four_gram_percentages, end="\t")
		print(diff_for_five_gram_percentages, end="\t")
		print(diff_for_six_gram_percentages, end="\t")
		print(average_suicide_ngram_percentage, end="\t")
		print(average_nonsuicide_ngram_percentage, end="\t")
		print(diff_between_ngram_percentage_averages, end="\t")
